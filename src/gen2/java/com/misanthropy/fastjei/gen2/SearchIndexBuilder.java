package com.misanthropy.fastjei.gen2;
...
public final class SearchIndexBuilder {
	...
	public static List<PrefixedSearchable<IListElementInfo<?>, IListElement<?>>> build(
		...
		if (parallel) {
			try {
				...
				Fastjei.LOGGER.error("[FastJEI] Parallel search index failed - rebuilding..", e.getCause());
	...
	private static Collection<String> safeGetStrings(
		...
		try {
			...
			Fastjei.LOGGER.debug("[FastJEI] getStrings failed on an ingredient, skipping it for this search prefix: {}", t.toString());
